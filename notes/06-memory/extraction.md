...
1. **Trigger** — stop hook fires `extractMemories` after the main model's final response
...
- **Forked agent shares prompt cache** — because it's a fork of the main conversation, the API call reuses the cached context. This makes the extraction nearly free in input tokens.
